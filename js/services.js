        ...
        allServices.forEach(([serviceKey, serviceItems]) => {
            ...
            <div>
                ...
                ${serviceItems["services"].map(item => {
                    ...
                    return`
                        <div style="border-radius: 10px; background-color: rgba(255, 255, 255, 0.5); padding: 10px; display: grid; gap: 5px;">
                        <h3>${item.name}</h3>
                        <table>
                        <tr><td>Price:</td><td>${price_label}</td></tr>
                        <tr><td>Expected Days of Service:</td><td>${item.ExpectedDaysOfService}</td></tr>
                        <tr><td>Service Detail:</td><td>${item.detail}</td></tr>
                        </table>
                        </div>
                    `
                }).join('')}
